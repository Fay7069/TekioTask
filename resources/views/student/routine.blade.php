...
@if($isLocked)
    ...
    {{-- Task is locked: threshold reached, no micro-steps available,
         teacher has been notified. Waiting for the teacher to Skip
         this task before the student can continue. Deliberately no
         timer/Done/Need Help here — retrying would just repeat the
         same failure and re-notify the teacher. --}}
...
@else
    ...
    <div class="task-progress-indicator">
        Task {{ $currentTaskNum }} of {{ $totalTasks }}
    </div>
...
const TOTAL_SEC  = {{ $task->estimated_duration_seconds }};
...
async function markDone() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            setTimeout(() => { window.location.href = '{{ route("student.routine") }}'; }, 1200);
        } else {
...
async function markHelp(calledByTimer = false) {
    ...
    try {
        const res = await fetch('{{ route("student.task.fail") }}', {
            method : 'POST',
            headers: {
                'Content-Type' : 'application/json',
                'X-CSRF-TOKEN' : CSRF_TOKEN,
        ...
        if (data.adapt) {
            ...
            setTimeout(() => { window.location.href = '{{ route("student.microstep") }}'; }, 1200);
        } else if (data.stop) {
            ...
            setTimeout(() => { window.location.href = '{{ route("student.routine") }}'; }, 1500);
