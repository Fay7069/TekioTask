...
<?php $__env->startSection('page-title', 'Class Dashboard'); ?>
...
async function fetchMapStatus() {
    try {
        const res  = await fetch('<?php echo e(route("teacher.map.status")); ?>', {
            headers: { 'Accept': 'application/json' }
        });
        if (res.status === 401) { window.location.href = '<?php echo e(route("login")); ?>'; return; }
...
async function fetchAlerts() {
    try {
        const res  = await fetch('<?php echo e(route("teacher.notifications.unread")); ?>', {
            headers: { 'Accept': 'application/json' }
        });
...
async function doSkip(userId, taskId) {
    try {
        const res = await fetch('<?php echo e(route("teacher.task.skip")); ?>', {
            method: 'POST',
            headers: {
...
<?php echo $__env->make('layouts.teacher', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\TekioTask\resources\views/teacher/dashboard.blade.php ENDPATH**/ ?>